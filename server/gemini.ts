...
export async function generate(difficulty: number): Promise<{
    problem: string;
    rest: string;
}> {
    ...
    const prompt = `あなたは「日本の大学入試数学問題作問の専門家」として、指定された難易度の数学模試問題を作成してください。以下の5段階プロセスで高品質な問題を生成し、最後に必ず自己検証を行ってください。
    ...
    ### 難易度調整指針
    - **基礎レベル (1)**：教科書レベル+α、確実な理解を問う
    - **標準レベル (2) **：入試標準問題、複数分野の融合
    - **応用レベル (3, 4の二段階) **：難関大学レベル、創造的思考を要求
    - **発展レベル (5)** ：最難関大学レベル、数学的洞察力を問う
